...
class State {
    ...
    {
        stream << (int)state.m_dCode;
        stream << (int)state.m_gCode;
        stream << (int)state.m_imgPolarity;
        stream << (int)state.m_interpolation;
        stream << (int)state.m_type;
        stream << (int)state.m_quadrant;
        stream << (int)state.m_region;
        stream << state.m_aperture;
        stream << state.m_lineNum;
        stream << state.m_curPos;
        stream << (int)state.m_mirroring;
    ...
    friend QDataStream& operator>>(QDataStream& stream, State& state)
    {
        int val;
        stream >> val;
        state.m_dCode = (DCode)val;
        stream >> val;
        state.m_gCode = (GCode)val;
        stream >> val;
        state.m_imgPolarity = (ImagePolarity)val;
        stream >> val;
        state.m_interpolation = (InterpolationMode)val;
        stream >> val;
        state.m_type = (PrimitiveType)val;
        stream >> val;
        state.m_quadrant = (QuadrantMode)val;
        stream >> val;
        state.m_region = (RegionMode)val;
        stream >> state.m_aperture;
        stream >> state.m_lineNum;
        stream >> state.m_curPos;
        stream >> val;
        state.m_mirroring = (Mirroring)val;
        stream >> state.m_scaling;
        stream >> state.m_rotating;
        return stream;
    }
